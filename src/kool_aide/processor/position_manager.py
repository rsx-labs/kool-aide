...
from kool_aide.db_access.dbhelper.position_helper import PositionHelper
...
class PositionManager:
    def __init__(self, logger: CustomLogger, config: AppSetting,
        ...
        self._db_helper = PositionHelper(
            self._logger, 
            self._config, 
            self._connection
        )
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.position_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[12]:
