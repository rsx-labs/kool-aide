...
from kool_aide.db_access.dbhelper.skills_helper import SkillsHelper
...
class SkillsManager:
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.skills_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[10]:
    ...
    def _add(self, data, argument: CliArgument):
        self._log(f"adding new skills: [{data['DESCR']}]")
        if self._is_ok_to_insert(data):
            ...
            if result:
                print_to_screen(f'Successful inserted new skills: {data["DESCR"]}', argument.quiet_mode)
                return True, ''
            else:
                print_to_screen(f'Failed creating new skills: {data["DESCR"]}', argument.quiet_mode)
                return False, error      
        else:
            print_to_screen(f'Failed creating new skills: {data["DESCR"]}', argument.quiet_mode)
