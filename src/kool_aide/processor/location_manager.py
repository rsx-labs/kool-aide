...
from kool_aide.db_access.dbhelper.location_helper import LocationHelper
...
class LocationManager:
    def __init__(self, logger: CustomLogger, config: AppSetting,
        ...
        self._db_helper = LocationHelper(
            self._logger, 
            self._config, 
            self._connection
        )
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.location_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[14]:
    ...
    def _add(self, data, argument: CliArgument):
        self._log(f"adding new location: [{data['LOCATION']}]")
        if self._is_ok_to_insert(data):
            ...
            if result:
                print_to_screen(f'Successful inserted new location: {data["LOCATION"]}', argument.quiet_mode)
                return True, ''
            else:
                print_to_screen(f'Failed creating new location: {data["LOCATION"]}', argument.quiet_mode)
                return False, error      
        else:
            print_to_screen(f'Failed creating new location: {data["LOCATION"]}', argument.quiet_mode)
