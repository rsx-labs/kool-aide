...
from kool_aide.db_access.dbhelper.status_helper import StatusHelper
...
class StatusManager:
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.status_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[11]:
    ...
    def _add(self, data, argument: CliArgument):
        self._log(f"adding new status list: [{data['STATUS_NAME']}]")
        if self._is_ok_to_insert(data):
            ...
            if result:
                print_to_screen(f'Successful inserted new status list: {data["STATUS_NAME"]}', argument.quiet_mode)
                return True, ''
            else:
                print_to_screen(f'Failed creating new status list: {data["STATUS_NAME"]}', argument.quiet_mode)
                return False, error      
        else:
            print_to_screen(f'Failed creating new status list: {data["STATUS_NAME"]}', argument.quiet_mode)
