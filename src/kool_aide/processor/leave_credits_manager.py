...
from kool_aide.db_access.dbhelper.leave_credits_helper import LeaveCreditsHelper
...
class LeaveCreditsManager:
    def __init__(self, logger: CustomLogger, config: AppSetting,
        ...
        self._db_helper = LeaveCreditsHelper(
            self._logger, 
            self._config, 
            self._connection
        )
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.leave_credits_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[9]:
    ...
    def _add(self, data, argument: CliArgument):
        self._log(f"adding leave credits for employee: [{data['EMP_ID']}]")
        if self._is_ok_to_insert(data):
            ...
            if result:
                print_to_screen(f'Successful inserted leave credits for employee: {data["EMP_ID"]}', argument.quiet_mode)
                return True, ''
            else:
                print_to_screen(f'Failed creating leave credits for employee: {data["EMP_ID"]}', argument.quiet_mode)
                return False, error      
        else:
            print_to_screen(f'Failed creating leave credits for employee: {data["EMP_ID"]}', argument.quiet_mode)
