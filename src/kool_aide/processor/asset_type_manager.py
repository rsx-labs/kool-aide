...
from kool_aide.db_access.dbhelper.asset_type_helper import AssetTypeHelper
...
class AssetTypeManager:
    def __init__(self, logger: CustomLogger, config: AppSetting,
        ...
        self._db_helper = AssetTypeHelper(
            self._logger, 
            self._config, 
            self._connection
        )
    ...
    def _log(self, message, level=3) -> None:
        self._logger.log(f"{message} [processor.asset_type_manager]", level)
    ...
    def retrieve(self, arguments: CliArgument)->(bool, str):
        ...
        if arguments.model == SUPPORTED_MODELS[8]:
    ...
    def _add(self, data, argument: CliArgument):
        self._log(f"adding new asset type:  [{data['TYP_DESCR']}]")
        if self._is_ok_to_insert(data):
            result, error = self._db_helper.insert(
                data['TYP_DESCR']
            )
            if result:
                print_to_screen(f'Successful inserted new asset type {data["TYP_DESCR"]}', argument.quiet_mode)
                return True, ''
            else:
                print_to_screen(f'Failed creating new asset type {data["TYP_DESCR"]}', argument.quiet_mode)
                return False, error      
        else:
            print_to_screen(f'Failed creating asset type {data["TYP_DESCR"]}', argument.quiet_mode)
