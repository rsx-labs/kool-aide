...
class AssetManufacturerHelper:
    ...
    def _log(self, message, level=3):
        self._logger.log(f"{message} [db_access.dbhelper.asset_manufacturer_helper]", level)
    ...
    def insert(self, description) -> bool:
        try:
            command = self._connection.entity.insert().values(
                MAN_DESCR = description
            )
